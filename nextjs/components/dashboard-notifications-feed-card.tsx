...
import { getAlmaPortalMessagesFeed, refreshAlmaPortalMessagesFeed } from "../lib/portal-api";

function toMessage(error: unknown): string {
  return error instanceof Error ? error.message : "The Alma notifications feed could not be loaded.";
}

export function DashboardNotificationsFeedCard() {
  ...
  useEffect(() => {
    ...
    async function loadFeed() {
      try {
        const result = await getAlmaPortalMessagesFeed();
        if (isCancelled) {
          return;
        }
        setFeed(result);
  ...
  async function handleRefresh() {
    ...
    try {
      const refreshed = await refreshAlmaPortalMessagesFeed();
      setFeed(refreshed);
      setError("");
    } catch (refreshError) {
